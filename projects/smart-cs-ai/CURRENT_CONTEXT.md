...
- 2026-08-22 财务报销助手新增校验：
  ...
  - 触发时会发消息，理由使用真实值：`发票抬头（实际识别抬头）与付款主体（OA付款主体）不一致`；当前调试期临时只推送康鹏。
...
- 地下水登记台账管理列表返回地址已修复：
  ...
  - 已执行 `cd frontend && ./build.sh` 通过，Node `v20.20.2`
- 财务报销图片发票识别继续补强：
  - 通义视觉识别会对图片/PDF 首页按 `0/90/180/270` 四个角度尝试，选择票号、抬头、税号字段更完整的角度。
  - 二维码票号识别优先使用视觉判定正常的角度。
  - 企业微信校验通知改为普通文本消息，付款主体不一致消息与事件记录均写真实发票抬头和付款主体，不再出现 `****`。
  - 正式容器临时同步后验证 `1212006` 附件 `1172607/1172609` 均可识别出票号。
  - 发票附件现在必须能实际解码出二维码内容；没有二维码内容的 PDF/图片直接跳过，不进抬头校验、不推消息。
  - 同一个附件命中多个校验失败时只发第一条消息，不再同时推“付款主体不一致”和“发票抬头有误”。
- 校验通知改为企业微信 markdown 消息，表单地址显示为 `[点击详情](OA表单URL)` 文字链。
- 营销项目备注中仅由斜线组成的占位内容（如 `/`、`////`）统一按空备注处理，不显示备注图标或查看弹窗；前端 `./build.sh` 已通过。
