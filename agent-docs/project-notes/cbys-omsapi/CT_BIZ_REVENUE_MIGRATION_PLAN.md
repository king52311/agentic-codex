...
`djwms` 表的所有数据必须在新库中都有，业务数据、用户、账户、表具、账单、流水、历史记录优先按它同步。

### 3. 新功能以 `djwms_new` 为准

结构参照 `djwms_new` 做适配，新抄表、出账、结算、配置、菜单权限、定时任务等功能表结构和初始化数据，按 `djwms_new` 补齐。

NB 设备、NB 订单、NB 端口映射等表也按 `djwms_new` 结构适配后同步进新库，不能漏。
...
- 表册、计划、活动、出账数据不迁移，目标库清空后保持为空；
- 余额、扣款、费用和流水统一以 `djwms` 为准。

数据合并脚本要求做成幂等模板：

- 先按主键或业务唯一键判断是否存在；
- 直接执行 upsert，存在则更新最新值，不存在则插入；
- 不做删除；
- 核心表优先覆盖，其他表后续按同一模式补充。
