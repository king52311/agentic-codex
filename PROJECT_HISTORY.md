...
- 增加通用经验沉淀规则：处理 `agentic-codex` 相关工程时，将已验证有效的排查思路、修复方法和经验结论及时写入公共规则，持续积累并降低后续 bug 风险。
...
- 已在 `omsapi/db/migrations/ct-biz-revenue/` 下补齐迁移脚本骨架，包含预检、结构补齐、基础数据合并、新功能配置合并和校验五个步骤，并同步更新执行说明。
- 结构补齐脚本已改为通用比对式，后续 `djwms_new` 新增表、字段、索引时无需改脚本本身，直接重跑即可。
- `02_merge_djwms_base_data.sql` 已改为幂等合并模板，按主键或业务唯一键插入/更新，不做删除，方便后续逐表补齐。
